...
import { QUERY } from 'src/components/ScheduleBuilderCell/ScheduleBuilderCell'
...
const ScheduleBuilderPage = () => {
  ...
  function publish() {
    data.aircrafts.map((aircraft) => {
      ...
      aircraft.spareFlyers.map((spare) => {
        publishSpare({
          variables: {
            spare_id: spare.id,
          },
    ...
    <DndProvider backend={HTML5Backend}>
      <Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
          <Box sx={{ width: '15vw', marginLeft: '25px', marginTop: '45px' }}>
            <PexSortiesCell
              start={start}
              end={end}
